...
class HTMLTree:
    def __init__(self):
        ...
        self.id2elementNode: dict = {}
        self.uniqueId2nodeId: dict = {}
        self.nodeId2uniqueId: dict = {}
    ...
    def build_node(node, idx: int) -> ElementNode:
        ...
        elementNode["data-unique-id"] = elementNode["attributes"].get("__elementid")
    ...
    def init_html_tree(self, root) -> None:
        ...
        while node_queue:
            ...
            self.id2elementNode[str(node_id)] = element_node
            if element_node.get("data-unique-id"):
                self.uniqueId2nodeId[element_node.get("data-unique-id")] = node_id
                self.nodeId2uniqueId[str(node_id)] = element_node.get("data-unique-id")
            node_id += 1
    ...
    def prune_tree(self) -> str:
        ...
        for nodeId in result:
            if self.is_valid(nodeId) or self.valid[nodeId] is True:
                ...
                rawNode = self.id2rawNode[str(nodeId)]
                if rawNode.getparent() is not None:
    ...
    def build_dom_tree_our_format(self) -> str:
        root = self.pruningTreeNode[0]
        stack = [root]
        contents = ""
        num = 0
        while stack:
            ...
            if self.valid[node["nodeId"]] is True:
                ...
                if content_text != "":
                    ...
                    _, tag_idx = self.get_tag_name(node)
                    if tag_name.lower() != "statictext":
                        num += 1
                        self.nodeDict[num] = tag_idx
                        contents += (
