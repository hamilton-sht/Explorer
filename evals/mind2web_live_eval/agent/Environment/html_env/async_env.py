...
class AsyncHTMLEnvironment:
    ...
    async def assign_unique_ids(self):
        ...
        await self.page.evaluate("""
            () => {
                ...
                let getInteractiveRects = function(element) {
                    let results = {};
                ...
                elements.forEach((element, index) => {
                    ...
                    let bbox = getInteractiveRects(element);
                    element.setAttribute('bbox', JSON.stringify(bbox));
                });
            }
        """)
        '''
        with open(os.path.join("mind2web_live_eval", "agent", "Utils", "page_script_6.js"), "rt") as fh:
            await self.page.evaluate(fh.read())
    ...
    async def get_obs(self) -> Union[str, Tuple[str, str]]:
        ...
        try:
            ...
            if self.args.omit_tab_name:
                observation = dom_tree
            else:
        ...
        if self.mode in ["d_v", "dom_v_desc", "vision_to_dom"]:
            ...
            return (observation, observation_VforD, self.page, self.tree)
        else:
            return observation
    ...
    async def fill_form(self, action):
        logger.info(f"inside fill_form action: {action}")
        try:
            label, element_id = self.tree.get_tag_name(
                self.tree.elementNodes[action["element_id"]])
            action.update({"element_id": element_id,
