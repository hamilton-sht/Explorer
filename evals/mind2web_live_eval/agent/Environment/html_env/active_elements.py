...
class ActiveElements:
    ...
    def is_visiable(element: ElementNode, only_child_check=True):
        ...
        if only_child_check:
            ...
            if visibility and (
                "visibility: hidden" in visibility
                or "visibility: collapse" in visibility
            ):
                return False
    ...
    def is_interactive(element: ElementNode):
        ...
        if tag in ["select", "option"] and element["attributes"].get("disabled"):
            return False
        if tag in ["input", "textarea", "button", "a"] and element["attributes"].get(
            "disabled"
        ):
            return False
        return True
    ...
    def is_valid_element(element: ElementNode) -> bool:
        return ActiveElements.is_interactive(element) and ActiveElements.is_visiable(
            element
        )
