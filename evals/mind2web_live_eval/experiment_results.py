...
def get_result(args, input_json_path):
...
def read_json_result(file_path):
    ...
    for items in data:
        ...
        try:
            ...
            data_dic["steps"] = []
            data_dic["task_score"] = "0 / {}".format(len(items["evaluation"]))
            data_dic["task_score_rate"] = 0.0

        # print(data_dic["task_score"], data_dic["task_score_rate"], len(items["evaluation"]))
...
def get_evaluate_result(args, input_result_path):
    out_file_path = get_result(args, input_result_path)
    evaluate(args=args, file_path=out_file_path)
