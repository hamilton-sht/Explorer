...
def generate_acc_tree(soup, result=[], eleid_attr_dict={}):
    ...
    for element in soup.find_all(recursive=False):
        ...
        if backend_node_id:
            ...
            result.append(acc_tree_line)

        # print('element.find_all(False) = ', element.find_all(recursive=False))
        if element.find_all(recursive=False):  # Recursively handle children
            generate_acc_tree(element, result)
