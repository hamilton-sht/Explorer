...
class GraphQLClient:
    ...
    def login(self):
        ...
        payload = {
            ...
            "variables": {"password": self.password, "username": self.username},
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Failed to login and retrieve auth token: {response.status_code} {response.text}"
            )
    ...
    def export_atom_flows(self, challenge_id, save_path):
        ...
        if response.status_code == 200:
            ...
            if (
                ...
                and "adminExportAtomFlowsOfChallenge" in response_data["data"]
            ):
                zip_file_url = response_data["data"]["adminExportAtomFlowsOfChallenge"]
                self.download_and_extract_zip_file(zip_file_url, save_path)
        else:
            raise Exception(
                f"Failed to export atom flows: {response.status_code} {response.text}"
            )
    ...
    def download_and_extract_zip_file(url, save_path):
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Failed to download ZIP file: {response.status_code} {response.text}"
            )
    ...
    def _validate_file_path(file_path):
        if not os.path.isfile(file_path):
            raise ValueError(
                f"The file path {file_path} does not exist or is not a file."
            )
    ...
    def _validate_save_path(save_path):
        if not os.path.isdir(save_path):
            raise ValueError(
                f"The save path {save_path} does not exist or is not a directory."
            )
...
if __name__ == "__main__":
    ...
    parser_upload.add_argument(
        "--file-path", required=True, help="Path to the file to be uploaded"
    )
    parser_upload.add_argument(
        "--challenge-id", required=True, help="Challenge ID for the upload"
    )
    parser_upload.add_argument("--name", required=True, help="Name for the upload")
    parser_upload.add_argument(
        "--base-model", required=True, help="Base model information for the upload"
    )

    # Subparser for the download command
    parser_download = subparsers.add_parser("download", help="Download atom flows")
    parser_download.add_argument(
        "--challenge-id", required=True, help="Challenge ID for the download"
    )
    parser_download.add_argument(
        "--save-path", required=True, help="Path to save the downloaded file"
    )
    ...
    if args.command == "upload":
        client.upload_file(
            args.name, args.base_model, args.file_path, args.challenge_id
        )
