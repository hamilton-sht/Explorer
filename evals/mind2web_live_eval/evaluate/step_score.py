...
class URLEvaluator(StepEvaluator):
    ...
    @staticmethod
    async def url_semantic_match(input_url, semantic_method, key=False, args=None):
        if key:
            try:
                parsed_url = urlparse(input_url)
                url_params = parse_qs(parsed_url.query)
    ...
    @staticmethod
    def url_semantic_match_sync(input_url, semantic_method, key=False, args=None):
        if key:
            try:
                parsed_url = urlparse(input_url)
                url_params = parse_qs(parsed_url.query)
...
class MatchFunction:
    ...
    async def semantic_match(input_answer, semantic_method, args) -> float:
        ...
        for i in range(3):
            try:
                ...
                response, _ = await semantic_match_llm_request(semantic_request, args)
    ...
    def semantic_match_sync(input_answer, semantic_method, args) -> float:
        ...
        for i in range(3):
            try:
                # response, _ = await GPT35.request(semantic_request)
                response, _ = semantic_match_llm_request_sync(semantic_request, args)
                score = re.findall("```(.*?)```", response, re.S)[0]
                score = eval(score)
                # Limit the score between 0 and 1
                score = max(0, min(1, score))
