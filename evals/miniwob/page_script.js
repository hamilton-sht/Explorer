var MultimodalWebSurfer = MultimodalWebSurfer || (function() {
    ...
    let annotateXPath = function(element) {
        ...
        //     return `//${element.tagName.toLowerCase()}[fill()="${element.getAttribute("fill")}"]`;
        // }

        if (element.id) {
            return `//${element.tagName.toLowerCase()}[@id="${element.id}"]`;
        }
        else if (element.tagName.toLowerCase() === "option" && element.textContent.trim()) {
            return `//${element.tagName.toLowerCase()}[text()="${element.textContent.trim()}"]`;
        }
        
        else if (element.hasAttribute("fill")) {
            if (element.textContent.trim()){
                return `//*[local-name()="${element.tagName.toLowerCase()}" and @fill="${element.getAttribute("fill")}" and text()="${element.textContent.trim()}"]`;
            }
            else {
                return `//*[local-name()="${element.tagName.toLowerCase()}" and @fill="${element.getAttribute("fill")}"]`;
            }
            //*[local-name()="circle" and @fill=
        }

        else if (element.tagName.toLowerCase() === "text" && element.textContent.trim()) {
            return `//*[local-name()="text" and normalize-space(text())="${element.textContent.trim()}"]`;
        } 
    ...
    let getApproximateAriaRole = function(element) {
        ...
        if (element.hasAttribute("fill")) {
            tag = tag + ", fill=" + element.getAttribute("fill");
        }
